...
$(document).ready(function() {
	...
	$('#submit_button').click(function(Event){
		if (ChannelImages.CFields.find('tr.ImageQueue div.Done').length > 0){
	...
	ChannelImages.CFields.delegate('tr.Image select', 'change', ChannelImages.TriggerChangeFile);
	ChannelImages.CFields.delegate('tr.Image textarea, tr.Image input', 'blur', ChannelImages.TriggerChangeFile);
	ChannelImages.CFields.delegate('tr.Image input', 'keydown', function(event){ if (event.keyCode == 13) return false;  });
...
ChannelImages.InitFields = function(){
	...
	// Activate Sortable
	ChannelImages.CFields.find('tbody.AssignedImages').sortable({axis:'y', cursor:'move', opacity:0.6, handle:'.ImageMove',
	...
	// Add some handlers
	ChannelImages.CFields.find('tbody.AssignedImages').delegate('a.ImageCover', 'click', ChannelImages.TogglePrimaryFile);
	ChannelImages.CFields.find('tbody.AssignedImages').delegate('a.ImageProcessAction', 'click', ChannelImages.OpenPerImageAction);
	ChannelImages.CFields.find('tbody.AssignedImages').delegate('a.ImageEdit', 'click', ChannelImages.OpenImageEdit);
	ChannelImages.CFields.find('tbody.AssignedImages').delegate('a.ImageDel', 'click', ChannelImages.DeleteImage);
...
ChannelImages.AddNewFile = function(JSONOBJ, FIELD_ID, Sync){
	...
	// Add it
	$('#ChannelImages_'+FIELD_ID).find('tbody.AssignedImages').append(HTML);
	$('#ChannelImages_'+FIELD_ID).find('tr.NoImages').hide();

	var JustAppended = $('#ChannelImages_'+FIELD_ID).find('tbody.AssignedImages').find('tr.Image:last');
...
ChannelImages.TriggerChangeFile = function(e){
	...
	ChannelImages.ChangeFile(Parent.data('field'), Parent.find(':input').val(), Parent.closest('tr'));
};
...
ChannelImages.FilesRemaining = function(FIELD_ID){
	var TotalFiles = $('#ChannelImages_'+FIELD_ID).find('tbody.AssignedImages tr.Image').not('.deleted').length;
...
ChannelImages.SyncOrderNumbers = function(FIELD_ID){
	...
	// Loop over all Files
	$('#ChannelImages_'+FIELD_ID).find('tbody.AssignedImages').find('tr.Image').each(function(Fileindex, Elem){
		var FILETD = $(Elem);
	...
	// Odd/Even
	$('#ChannelImages_'+FIELD_ID).find('tr.Image').removeClass('odd');
	$('#ChannelImages_'+FIELD_ID).find('tr.Image:odd').addClass('odd');
};

//********************************************************************************* //

ChannelImages.TogglePrimaryFile = function(Event){

	// Store!
	var TableParent = $(Event.target).closest('table');
	var Parent = $(Event.target).closest('tr');

	// Are we unchecking?
	var Uncheck = false;
	if ( $(Event.target).hasClass('StarIcon') ) Uncheck = true;

	// Find all files and remove the StarClass & Cover Value
	TableParent.find('tbody.AssignedImages').find('tr.Image').each(function(i, elem){
		$(elem).removeClass('PrimaryImage').find('a.ImageCover').removeClass('StarIcon');
...
ChannelImages.EditFileDetails = function(value, settings){
	...
	ChannelImages.ChangeFile(Field, value, $(this).closest('tr.Image'));
		...
		onComplete: function(){
			...
			Elem.find('.ImportImagesBtn').click(function(){
				...
				jQuery.post(ChannelImages.AJAX_URL, Params, function(rData){
					...
					jQuery('#ChannelImages_'+FIELD_ID).find('tr.ImagesQueue').append('<div class="Done"></div>');
...
ChannelImages.HTML5.UploadStart = function(FIELD_ID) {
	...
	$('#ChannelImages_' + FIELD_ID).find('a.StopUpload').click(function(){
		$('#ChannelImages_' + FIELD_ID).find('tr.ImageQueue div.File').not('div.Done').each(function(index,elem){
			var Elem = $(elem);
			xhr.abort();
...
ChannelImages.OpenPerImageAction = function(Event){
	...
	$.colorbox({
		innerWidth:550,
		innerHeight:400,
		html:$.base64Decode($('.PerImageActionHolder:first').text()),
		onComplete: function(){
...
ChannelImages.OpenImageEdit = function(Event){
	Event.preventDefault();
	...
	$(Event.target).closest('tr').after(Cloned);
