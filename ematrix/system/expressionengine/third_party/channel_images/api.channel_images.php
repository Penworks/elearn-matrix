...
class Channel_Images_API
{
	...
	{
		if (isset($data['field_id']) === FALSE) return FALSE;
		if (isset($data['entry_id']) === FALSE) return FALSE;
		...
		if (isset($data['image_url']) === TRUE)
		{
			...
			if (isset($this->valid_mime[ $data['extension'] ]) === FALSE) return FALSE;
		}
		...
		if (isset($data['image_data']) === TRUE || isset($data['image_url']) === TRUE)
		{
			...
			if (isset($data['filename']) === FALSE OR $data['filename'] == FALSE) return FALSE;
			...
			elseif ($data['extension'] == 'jpg') imagejpeg($img, $this->temp_dir.$data['filename']);
			elseif ($data['extension'] == 'jpg') imagejpeg($img, $this->temp_dir.$data['filename']);
		...
		// Last Check
		if (file_exists($this->temp_dir.$data['filename']) === FALSE) return FALSE;
		if (isset($this->valid_mime[ $data['extension'] ]) === FALSE) return FALSE;
